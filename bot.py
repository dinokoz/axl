...
# Constant
DISCORD_TOKEN = "NTI2NzQ0NTQzNTgwNjUxNTIw.Dx5Tlg.WBWEpPNIKoEYovlNs05QHMEyBzw"

FORTNITE_API_KEY = 'a899bf11-04e2-42a7-844d-f6f82fc6c64c'
...
async def on_message(message):
    ...
    if message.content.startswith('/patch'):
        await client.send_message(message.channel, 'Последняя обнова: https://www.epicgames.com/fortnite/ru/news')
    ...
    if message.content.startswith("/rank"):
        ...
        if ratio >= WOOD_B and ratio <= WOOD_E:
            ...
            msg = ("{0.author.mention} братик, у тебя ранк " + role.name).format(message)
            ...
            msg = ("{0.author.mention} Прости, но я не нашел игрока с таким ником, скорей всего у тебя разные ники в фортнайте и в дискорде! Используй комманду ```/nick ТВОЙ_НИК_В_ФОРТНАЙТЕ``` И пробуй снова").format(message)
            await client.send_message(message.channel, msg)
        elif ratio == -2:
            msg = ("{0.author.mention} Кривые сервера фортнайта оффлайн, попробуй позже!").format(message)
            await client.send_message(message.channel, msg)
